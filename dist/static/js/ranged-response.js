...
class RangedResponse {
    ...
    return this.getBestCacheMatch(request).then(cacheName => {
      ...
      return caches.match(`${request.url}_0`, {cacheName: cacheName})
        .then(firstChunk => {
          ...
          const startIndex = Math.floor(start / CHUNK_SIZE);
          const endIndex = Math.floor(end / CHUNK_SIZE);
  ...
  static _createFromChunks (request, cacheName) {
    try {
      ...
      const startIndex = Math.floor(start / CHUNK_SIZE);
      const endIndex = Math.floor(end / CHUNK_SIZE);
      const offset = startIndex * CHUNK_SIZE;
      ...
      const bufferSize = (endIndex - startIndex + 1) * CHUNK_SIZE;
